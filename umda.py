...
def calculate_fitness(individual, X_train, X_test, y_train, y_test):
    ...
    # Perform a randomized search with cross-validation (you can use GridSearchCV as well)
    ...
    # Get the best SVM classifier from the randomized search
    best_clf = random_search.best_estimator_

    # Fit the best classifier on the training data and make predictions
    best_clf.fit(X_train_sel, y_train)
    y_pred = best_clf.predict(X_test_sel)

    # Calculate the balanced accuracy score for the best classifier
